...
class AccusedRepository extends BaseRepository {
  ...
  async findRepeatOffenders({ minCases = 2, limit = 100 } = {}) {
    ...
    // Catalyst's ZCQL does not honor column aliases on aggregate
    // expressions — the raw key comes back as the literal expression
    // string (e.g. "COUNT(AccusedName)") instead of "caseCount", so
    // it's remapped here to the name the rest of the app expects.
